...
| double (n : Nat) : ExampleSeqContains n → ExampleSeqContains (2 * n)
| mod (n : Nat) : ExampleSeqContains (4 * n + 1)

lemma not_example_seq_contains_zero : ¬ ExampleSeqContains 0 := by
  generalize h₀ : 0 = n
  by_contra h
  induction h with
  | double n' _ h' =>
...
lemma not_example_seq_contains_double {n : Nat} (h : ¬ ExampleSeqContains n) :
  ¬ ExampleSeqContains (2 * n) := by
  generalize hn : 2 * n = n'
  by_contra h'
  cases h' with
  | double n'' h'' =>
    ...
    exact h''
  | mod n'' =>
    absurd show 0 ≠ 1 by simp
    rw [show 0 = (2 * n) % 2 by simp, show 1 = (2 * (2 * n'') + 1) % 2 by simp, ← mul_assoc]
    simp only [Nat.reduceMul]
    congr

lemma not_example_seq_contains_three_mod {n : Nat} : ¬ ExampleSeqContains (4 * n + 3) := by
  generalize hn : 4 * n + 3 = n'
  by_contra h
  cases h with
  | double n'' h' =>
    absurd show 1 ≠ 0 by simp
    rw [show 1 = (2 * (2 * n) + 3) % 2 by simp, ← mul_assoc, show 0 = (2 * n'') % 2 by simp]
    simp only [Nat.reduceMul]
    congr
  | mod n'' =>
    absurd show 3 ≠ 1 by simp
    rw [show 3 = (4 * n + 3) % 4 by simp, show 1 = (4 * n'' + 1) % 4 by simp]
    congr
...
instance decExampleSeqContains : DecidablePred (ExampleSeqContains ·) := by
  ...
  | ind n f =>
    ...
    · refine .isFalse ?_
      rw [n_zero]
      exact not_example_seq_contains_zero
    by_cases n_even : n % 2 = 0
    · rw [show n = 2 * (n / 2) by sorry]
      cases f (n / 2) sorry with
      | isTrue h =>
        refine .isTrue ?_
        refine .double (n / 2) ?_
        exact h
      | isFalse h =>
        refine .isFalse ?_
        apply not_example_seq_contains_double
        exact h
    · let n' := n / 4
      by_cases n_mod : n % 4 = 1
      · refine .isTrue ?_
        rw [show n = 4 * n' + 1 by sorry]
        refine .mod ?_
      · refine .isFalse ?_
        --apply Nat.mod_lt
        sorry
...
instance decExampleSeqContains : DecidablePred (ExampleSeqContains ·) := by
  ...
  · refine .isFalse ?_
    ...
    exact not_example_seq_contains_zero
  by_cases n_even : n % 2 = 0
  · cases f (n / 2) (Nat.div_lt_self sorry sorry) with
    | isTrue h =>
...
lemma example_seq_contains (n : Nat) : ExampleSeqContains (exampleSeq n) := by
  cases n with
  | zero =>
    exact @Nat.find_spec (ExampleSeqContains ·) _ _
  | succ n' =>
    exact (@Nat.find_spec (fun m => m > exampleSeq n' ∧ ExampleSeqContains m) _ _).right
...
theorem example_seq_pos : ∀ n : Nat, 0 < exampleSeq n := by
  ...
  by_contra h
  cases example_seq_contains n with
  | half n' => sorry
  | mod n' h' => sorry
  sorry
