...
The contestants are asked to determine all real numbers $r$ such that there exists a sequence of
positive integers $a_0,a_1,a_2,\dots$ which satisfies the following three conditions:
* No number appears more than once in the sequence.
* No two different members of the sequence add up to a power of two.
* $a_n<r\cdot n$ for all $n\in\mathbb{N}$.
...
/-- The sequence `seq` only contains positive numbers and satisfies the three conditions for
`r≥2`. -/
...
/-- Assigns `n : ℕ` to `(m, l) : ℕ × ℕ` such that `n = 2 ^ m (2 * l + 1)` (for `n ≠ 0`). -/
def decomp : ℕ × ℕ :=
  let m := Nat.findGreatest (2 ^ · ∣ n) n
  let l := (n / 2 ^ m - 1) / 2
  (m, l)

/-- The inverse function of `decomp`. -/
...
/-! Show that `recomp` and `decomp` are in fact inverse functions: -/

lemma recomp_decomp (hn : n ≠ 0) : n = 2 ^ (decomp n).1 * (2 * (decomp n).2 + 1) := by
  unfold decomp
  simp
  set m := Nat.findGreatest (2 ^ · ∣ n) n
  rw [show m = (decomp n).1 by rfl]
...
lemma decomp_recomp' : let n := 2 ^ m * (2 * l + 1); Nat.findGreatest (2 ^ · ∣ n) n = m := by
  ...
  · generalize hm' : Nat.findGreatest (2 ^ · ∣ n) n = m'
    ...
    by_contra ha
    have hm' := hm'.right.left
    have hm' := hm' (Nat.ne_zero_of_lt ha)
    rw [← Nat.sub_add_cancel ha.le, pow_add, mul_comm] at hm'
    have hm' := Nat.dvd_div_of_mul_dvd hm'
    unfold n at hm'
    simp at hm'
    absurd (show ¬2 ∣ 2 * l + 1 by simp)
    refine Nat.dvd_of_pow_dvd ?_ hm'
...
lemma decomp_recomp : decomp (2 ^ m * (2 * l + 1)) = (m, l) := by
  unfold decomp
  rw [decomp_recomp' m l]
  simp

/-! Define the example sequence and show some auxillary properties: -/

/-- A number `n` will be contained in `exampleSeq` if and only if `ExampleSeqContains n`. -/
...
lemma example_seq_contains_gt : ∃ m ∈ setOf ExampleSeqContains, n < m := by
  ...
  simp
  rw [show 2 ^ n = 2 ^ n * (2 * 0 + 1) by simp, decomp_recomp]
  simp

lemma example_seq_infinite : (setOf ExampleSeqContains).Infinite := by
  apply Set.infinite_of_forall_exists_gt
  exact example_seq_contains_gt

/-- The sequence to be used as an example. -/
noncomputable def exampleSeq : ℕ → ℕ := Nat.nth ExampleSeqContains

/-- A computable alternative to `exampleSeq`. -/
...
/-! Prove that `exampleSeq` satisfies the conditions: -/

lemma example_seq_pos : 0 < exampleSeq n := by
  apply Nat.zero_lt_of_ne_zero
  exact example_seq_ne_zero n
...
lemma two_dvd_two_pow_sub (h : n₁ < n₂) : 2 ∣ 2 ^ (n₂ - n₁) := by
  show 2 ^ 1 ∣ 2 ^ (n₂ - n₁)
  rw [Nat.pow_dvd_pow_iff_le_right (show 1 < 2 by simp)]
  rw [Nat.one_le_iff_ne_zero, Nat.ne_zero_iff_zero_lt]
  apply Nat.zero_lt_sub_of_lt
  exact h
...
lemma example_seq_m_eq_m_of_sum_pow_two (hm : m₁ < m₂)
  ...
  have hm₁k : m₁ < k := example_seq_m_lt_k m₁ l₁ m₂ l₂ k h
  unfold recomp at h
  rw [← Nat.sub_add_cancel hm.le, ← Nat.sub_add_cancel hm₁k.le, pow_add, pow_add] at h
  conv at h => lhs; right; left; rw [mul_comm]
  conv at h => lhs; right; rw [mul_assoc]
  conv at h => rhs; rw [mul_comm]
  rw [← Distrib.left_distrib] at h
  simp at h
  absurd two_dvd_two_pow_sub m₁ k hm₁k
  rw [← h, ← even_iff_two_dvd]
  simp
  rw [Nat.odd_add, Nat.even_mul]
  simp
  rw [even_iff_two_dvd]
  exact two_dvd_two_pow_sub m₁ m₂ hm

lemma example_seq_l_eq_l_of_sum_pow_two (hl₁ : 2 ∣ l₁) (hl₂ : 2 ∣ l₂)
    (h : recomp m l₁ + recomp m l₂ = 2 ^ k) : l₁ = l₂ := by
  have hmk : m < k := example_seq_m_lt_k m l₁ m l₂ k h
  unfold recomp at h
  rw [← Nat.sub_add_cancel hmk.le, pow_add] at h
  conv at h => rhs; rw [mul_comm]
  rw [← Distrib.left_distrib, Nat.add_add_add_comm, ← Distrib.left_distrib] at h
  simp at h
  ...
  | 1 =>
    simp at h
    rw [Nat.add_eq_one_iff] at h
    cases h with
    | inl h => rw [h.right] at hl₂; simp at hl₂
    | inr h => rw [h.left] at hl₁; simp at hl₁
  | x + 2 =>
    conv at h => lhs; right; rw [show 2 = 2 * 1 by rfl]
    rw [← Distrib.left_distrib] at h
    rw [pow_add, mul_comm] at h
    simp at h
    absurd show ¬Even (l₁ + l₂ + 1) by
      rw [Nat.even_add_one, Nat.even_add, even_iff_two_dvd, even_iff_two_dvd]
      simp
      exact iff_of_true hl₁ hl₂
    rw [h, show x + 2 = x + 1 + 1 by simp, pow_add, mul_comm]
    simp
...
lemma example_seq_no_sum_two_pow (h : exampleSeq n₁ + exampleSeq n₂ = 2 ^ k) : n₁ = n₂ := by
  ...
  rw [recomp_decomp (exampleSeq n₂) (example_seq_ne_zero n₂)] at h ⊢
  set m₁ := (decomp (exampleSeq n₁)).1
  set l₁ := (decomp (exampleSeq n₁)).2
  set m₂ := (decomp (exampleSeq n₂)).1
  set l₂ := (decomp (exampleSeq n₂)).2
  have hm : m₁ = m₂ := by
    by_contra ha
  ...
  rw [hm] at h ⊢
  have hl : l₁ = l₂ := by
    refine example_seq_l_eq_l_of_sum_pow_two m₂ l₁ l₂ k ?_ ?_ h
    <;> apply example_seq_two_dvd_l _
  rw [hl]
...
/-- Injectively assigns a non-zero `Nat` that is not contained in `exampleSeq` to a smaller `Nat`
that is contained `exampleSeq`. -/
def matchNonMemberWithMember
  ...
  · calc
      _ < n.val := ?_
      _ < exampleSeq n' := ?_
    · rw [recomp_decomp n h.right.left]
      unfold m l
      by_contra ha
      simp at ha
...
lemma injective_match_non_member_with_member :
  ...
  have hn₁ := n₁.prop
  have hn₂ := n₂.prop
  rw [Finset.mem_filter, ne_comm] at hn₁ hn₂
  unfold matchNonMemberWithMember at h
  simp at h
  have h := congr_arg decomp h
  rw [decomp_recomp, decomp_recomp] at h
  simp at h
  apply Subtype.eq
  rw [recomp_decomp n₁ hn₁.right.left, recomp_decomp n₂ hn₂.right.left, ← h.left]
  simp
  refine Nat.sub_one_cancel ?_ ?_ h.right
  <;> rw [← Nat.ne_zero_iff_zero_lt]
  <;> apply ne_zero_of_mod_ne_zero _ 2
...
/-! ### No sequence exists for r<2 -/

variable {r : ℝ} {seq : ℕ → ℕ}
...
/-- Plug `n` into `seq` and - if the result exceeds `2 ^ k` - "mirror" it around `2 ^ k`.
By this, `seq n₁` and `seq n₂` are equal or add up to `2 ^ (k + 1)` if and only if `n₁` and `n₂`
are mapped to the same value by `mirrorSeq` (if `n₁ ≤ 2 ^ k` and `n₂ ≤ 2 ^ k`). -/
...
lemma lt_pigeon_hole (a b : ℕ) (h₁ : a ≤ b) (h₂ : ∀ c ≤ b, seq c < a) :
  ...
  let pigeonHoles := range a
  let pigeons := range (b + 1)
  let ⟨n, hn, m, hm, n_ne_m, f_n_eq_f_m⟩ :
      ∃ n ∈ pigeons, ∃ m ∈ pigeons, n ≠ m ∧ seq n = seq m := by
    ...
    · intros n hn
      ...
      simp
      apply h₂
      rw [Nat.le_iff_lt_add_one, ← mem_range]
      exact hn
  rw [mem_range, ← Nat.le_iff_lt_add_one] at hn hm
  use n
  refine ⟨hn, ?_⟩
  use m
...
lemma not_exists_k (h_seq : IsRSeq r seq) : ¬ ∃ k, ∀ n ≤ 2 ^ k, seq n < 2 ^ (k + 1) := by
  ...
  let ⟨n, hn, m, hm, n_ne_m, pair_n_m⟩ : ∃ n ≤ 2 ^ k, ∃ m ≤ 2 ^ k, n ≠ m ∧ mSeq n = mSeq m := by
    apply lt_pigeon_hole (2 ^ k) (2 ^ k) (by simp)
    intros n _
...
/-- The set of all real numbers such that a sequence satisfying the conditions exists. -/
